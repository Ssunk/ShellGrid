...
<section role="application" class:active class="terminal-pane" on:pointerdown={() => app.setActivePane(paneId)}>
  <header class="pane-header">
    ...
    <button class="icon-button" title="左右分割 (Ctrl+Alt+H)" on:click|stopPropagation={() => app.split(paneId, "horizontal")}>
      <SplitSquareHorizontal size={15} />
    </button>
    <button class="icon-button" title="上下分割 (Ctrl+Alt+V)" on:click|stopPropagation={() => app.split(paneId, "vertical")}>
      <SplitSquareVertical size={15} />
    </button>
    <button class="icon-button close-button" title="关闭窗格 (Ctrl+Alt+W)" on:click|stopPropagation={() => app.close(paneId)}>
      <X size={15} />
    </button>
  </header>
